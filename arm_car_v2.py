...
# 果梗角度 → J6 roll 的符號。上機若發現夾爪轉反，改成 -1.0
STEM_ANGLE_SIGN = +1.0

# 果梗仰角 → 接近軸往下傾的符號。+1.0 = 依果梗仰角往下壓(從上方斜斜下探)。
# 上機若發現它往「上」翹或戳反了，改成 -1.0。設成 0.0 可暫時關掉仰角、退回純水平。
PEDICEL_PITCH_SIGN = +1.0
...
class MathUtils:
    @staticmethod
    def calculate_grasp_and_approach(tomato_x, tomato_y, tomato_z, stem_angle_deg = 0.0,
                                     az_deg = 0.0, el_deg = 0.0, base_yaw = 0.0,
                                     gripper_length = 0.16, approach_dist = 0.1):
        # ★ 接近軸『垂直於果梗真實 3D 方向』(broadside，從側面切進去，才夾得牢)：
        #   1) 用 az+el 合成果梗方向向量 p(branch->calyx，垂直性只看方向、正負不影響)
        #   2) 取「水平朝目標」方向 h = [cos(base_yaw), sin(base_yaw), 0]
        #   3) 把 h 投影到『垂直 p 的平面』上(減掉沿 p 的分量) -> 得到既垂直果梗、又最接近水平朝目標的接近軸
        #   數學上 (h - (h·p)p) 必定垂直 p；果梗垂直下垂時會自然退回水平進去(=與果梗 90°)。
        az_p = math.radians(az_deg)
        el_p = math.radians(el_deg)
        p = np.array([
            math.cos(az_p) * math.cos(el_p),
            math.sin(az_p) * math.cos(el_p),
            -math.sin(el_p),
        ])
        p = p / (np.linalg.norm(p) + 1e-12)

        h = np.array([math.cos(base_yaw), math.sin(base_yaw), 0.0])   # 水平朝目標
        z_axis = h - np.dot(h, p) * p                                 # 投影到垂直 p 的平面
        nz = np.linalg.norm(z_axis)
        if nz < 1e-6:
            # 退化：果梗幾乎水平又正對目標，垂直方向湊不出來 -> 退回水平朝目標(此時剛好平行，屬邊界情況)
            z_axis = h.copy()
        else:
            z_axis = z_axis / nz

        # 依 z_axis 重建正交的 x/y 軸；接近軸幾乎垂直(和世界上方平行)時要換參考避免除以零
        world_up = np.array([0.0, 0.0, 1.0])
        x_axis = np.cross(world_up, z_axis)
        nx = np.linalg.norm(x_axis)
        if nx < 1e-6:
            # z 幾乎朝正下方，改拿世界 X 當參考、去掉沿 z 的分量後正規化
        ...
        base_rot = R.from_matrix(np.column_stack((x_axis, y_axis, z_axis)))
        # ★ 繞接近軸再轉一個 roll，讓夾爪開口對準果梗左右歪斜(這段邏輯不變)
        roll_rad = STEM_ANGLE_SIGN * math.radians(stem_angle_deg)
        final_rot = R.from_rotvec(roll_rad * z_axis) * base_rot
        qx, qy, qz, qw = final_rot.as_quat()

        # 抓取點/預備點沿傾斜後的接近軸往回退，直線下探自然變成「順著果梗斜斜往下」
        grasp_x = tomato_x - (gripper_length * z_axis[0])
        grasp_y = tomato_y - (gripper_length * z_axis[1])
        grasp_z = tomato_z - (gripper_length * z_axis[2])
        app_x = grasp_x - (approach_dist * z_axis[0])
        app_y = grasp_y - (approach_dist * z_axis[1])
        app_z = grasp_z - (approach_dist * z_axis[2])

        # 注意：tuple 第 8 個放 base_yaw(面對目標的基座方位)，是給 J1 約束穩定手腕用的，
        #       跟上面接近軸的水平方向(az)是兩回事，不可混用。
        grasp_target = (grasp_x, grasp_y, grasp_z, qx, qy, qz, qw, base_yaw)
        approach_target = (app_x, app_y, app_z, qx, qy, qz, qw, base_yaw)
...
class TM5MController:
    def __init__(self, node: Node, cb_group):
        ...
        self._cartesian_vel_scale = 0.1  # 直線放慢的預設縮放(execute_cartesian_path 會覆寫)
    ...
    def execute_cartesian_path(self, target_tuple, done_cb=None, velocity=0.1, accel=0.1):# 笛卡爾直線服務
        self._current_done_cb = done_cb
        ...
        # ★ 注意：Humble 版的 GetCartesianPath.Request 沒有 max_velocity_scaling_factor
        #   這個欄位(那是較新 moveit_msgs 才有)，所以速度不能在這裡設，設了會 AttributeError。
        #   改成先把這次要的縮放記起來，等軌跡算完再把時間軸拉長來放慢(見 _on_cartesian_planned)。
        #   velocity 越小越慢；還是覺得快就再往下調(例如 0.05)。
    ...
    def _on_cartesian_planned(self, future):
        ...
        # 把算好的直線路徑先放慢(拉長時間軸)，再塞進執行指令裡
        slowed = self._retime_trajectory(res.solution, self._cartesian_vel_scale)
        goal_msg = ExecuteTrajectory.Goal(trajectory=slowed)
        self._send_action_goal(self.exec_client, goal_msg)

    def _retime_trajectory(self, robot_traj, scale):
        # 把笛卡爾規劃好的軌跡整體放慢。scale<1 => 變慢。
        # 作法：時間軸拉長為 1/scale 倍；同時速度×scale、加速度×scale^2，維持運動學一致。
        # (Humble 的 GetCartesianPath 不吃速度縮放，只能在這裡事後處理)
        if scale >= 0.999:
            return robot_traj
        inv = 1.0 / scale
...
class TM5MTaskNode(Node):
    ...
    def target_callback(self, msg: PoseStamped):
        ...
        # 從四元數還原三個角度(視覺端用 euler(roll, elevation, azimuth) 以 'xyz' 打包)：
        #   roll      = 果梗左右歪 -> 夾爪繞接近軸的 roll
        #   elevation = 果梗往下傾多陡 -> 接近軸仰角
        #   azimuth   = 果梗水平方位 -> 接近軸水平朝向
        roll_rad, el_rad, az_rad = R.from_quat([q.x, q.y, q.z, q.w]).as_euler('xyz')
        stem_angle_deg = math.degrees(roll_rad)
        el_deg = math.degrees(el_rad)
        az_deg = math.degrees(az_rad)
        # J1 約束用的「面對目標方位」仍由手臂自己用位置算(跟接近軸的 azimuth 分開)
        base_yaw = math.atan2(pos.y, pos.x)
        self.get_logger().info(
            f'\n果梗抓取點 X:{pos.x:.3f}, Y:{pos.y:.3f}, Z:{pos.z:.3f}, '
            f'左右角:{stem_angle_deg:.1f}°, 方位az:{az_deg:.1f}°, 仰角el:{el_deg:.1f}°')

        # 用果梗真實 3D 方向(az/el)建接近軸；base_yaw 只給 J1 穩定手腕
        self.grasp_target, self.approach_target = MathUtils.calculate_grasp_and_approach(
            pos.x, pos.y, pos.z, stem_angle_deg=stem_angle_deg,
            az_deg=az_deg, el_deg=el_deg, base_yaw=base_yaw)
        
        # 開始執行狀態機：往預備點移動
        self.current_step = 'APPROACH'
        self.is_moving = True
        self.arm.control_gripper(0.008) 
    ...
    def on_action_completed(self, success):# 統一接收所有手臂動作完成的事件，並觸發下一步
        ...
        if self.current_step == 'INIT':# 完成初始定位
            ...
            # self._step_to_basket()   # ← 暫時註解掉：先不去籃子
            self.get_logger().info('[步驟 5] 退回點 A 完成！(暫時跳過籃子) 直接回初始位置')
    ...
    def _move_to_initial(self):
        self.is_moving = True
        target_radians = [math.radians(deg) for deg in [-90.0, -15.0, 130.0, -115.0, 90.0, 0.0]]
