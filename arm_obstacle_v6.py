...
class TM5MController:
    ...
    def execute_cartesian_path(self, target_tuple, done_cb=None, velocity=0.1, accel=0.1):# 笛卡爾直線服務
        self._current_done_cb = done_cb
        # 註：Humble 的 GetCartesianPath.Request 沒有 max_velocity_scaling_factor(設了會 AttributeError)，
        #     所以速度不在這裡設，改成先記住縮放值，等軌跡算完再把時間軸拉長來放慢(見 _on_cartesian_planned)。
...
class TM5MTaskNode(Node):
    ...
    def _move_to_initial(self):
        self.is_moving = True
        target_radians = [math.radians(deg) for deg in [0.0, -30.0, 115.0, -85.0, 90.0, 0.0]]
