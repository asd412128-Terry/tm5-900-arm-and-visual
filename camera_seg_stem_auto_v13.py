...
class VisionNode(Node):
    ...
    def color_callback(self, msg):
                    ...
                    if 0 <= cx_t < self.latest_depth_img.shape[1] and 0 <= cy_t < self.latest_depth_img.shape[0]:
                        ...
                        if len(valid_depths) > 0:
                            ...
                            if z_t > 0.01:
                                ...
                                # 框上座標文字移除，改到左上角面板統一顯示；ID 於配對後再標
        ...
        if len(detected_objects) > 0:
            ...
            for idx, obj in enumerate(detected_objects):
                ...
                # 框上只留 ID
                cv2.putText(cv_image, f"ID:{idx}", (int(b[0]), int(b[1]) - 8),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

            # 番茄框上只標「配對到的果梗 ID」(每顆番茄取最近的果梗)
            for t in detected_tomatoes:
                dists = [math.hypot(t['cx']-obj['cx'], t['cy']-obj['cy']) for obj in detected_objects]
                pid = int(np.argmin(dists)) if dists else -1
                tb = t['bbox']
                cv2.putText(cv_image, f"ID:{pid}", (int(tb[0]), int(tb[3]) + 18),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)

            # 左上角資訊面板(仿終端清單)
            self._draw_info_panel(cv_image, detected_objects, detected_tomatoes)
            ...
            if not self.is_processing and (now - self._last_scan_print) > 1.5:
                ...
                for idx, obj in enumerate(detected_objects):
                    print(f"  [ID:{idx}] Stem   X={obj['world_x']:.3f}, Y={obj['world_y']:.3f}, Z={obj['world_z']:.3f}°")
                          #, Angle={obj['angle']:.1f}°, Az={obj.get('az',0.0):.1f}°, El={obj.get('el',0.0):.1f}°")
                    # 找最近的番茄配對顯示
                    if len(tomatoes) > 0:
    ...
    def _draw_info_panel(self, img, stems, tomatoes):
        # 在畫面左上角畫一個仿終端的資訊面板（半透明黑底 + 逐行座標）
        lines = [f"Detected: {len(stems)} stems / {len(tomatoes)} tomatoes"]
        for idx, obj in enumerate(stems):
            lines.append(f"[ID:{idx}] Stem   X={obj['world_x']:.3f} Y={obj['world_y']:.3f} Z={obj['world_z']:.3f}")
            if len(tomatoes) > 0:
                dists = [math.hypot(obj['cx']-t['cx'], obj['cy']-t['cy']) for t in tomatoes]
                nt = tomatoes[int(np.argmin(dists))]
                lines.append(f"        Tomato X={nt['world_x']:.3f} Y={nt['world_y']:.3f} Z={nt['world_z']:.3f}")
        ...
        for i, ln in enumerate(lines):
            color = (0, 255, 255) if i == 0 else (255, 255, 255)  # 標題黃、其餘白
