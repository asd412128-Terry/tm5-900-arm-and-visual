...
class VisionNode(Node):
    def __init__(self):
        ...
        self.model = YOLO('/home/terry/yolo_tomato_complex/runs/detect/tomato_m_v3/weights/best.pt')  
    ...
    def color_callback(self, msg):
        ...
        # 防呆：四元數若為全零（TF 尚未 settle），代表旋轉無效，跳過這一幀避免 matmul 產生 nan
                ...
                if 0 <= cx_pixel < self.latest_depth_img.shape[1] and 0 <= cy_pixel < self.latest_depth_img.shape[0]:
                    ...
                    if len(valid_depths) > 0:
                        ...
                        if z_real > 0.01:
                            ...
                            f_avg = (fx + fy) / 2.0 
                            tomato_radius = (avg_pixel_size * z_real) / f_avg / 2.0 
                            ...
                            # world_point.point.z += (tomato_radius + 0.0)  # 抓番茄中心：z_center 已修正到中心，這裡不再往上墊半徑

                            # 防呆：若 TF 轉換結果含 nan/inf（手臂 TF 尚未 settle 或深度異常），跳過這顆番茄
                            ...
                            if not (np.isfinite(wx) and np.isfinite(wy) and np.isfinite(wz)):
                                self.get_logger().warn(f"座標含無效值 (nan/inf)，已跳過此目標 (像素 {cx_pixel},{cy_pixel})")
                                continue
    ...
    def auto_pick_thread(self):
        ...
        self.get_logger().info(f"「自動模式」掃描完成，偵測到 {len(targets)} 個番茄目標。")
        
        selected_target = None
        max_reach = 0.9

        # 巡檢排序好的番茄，尋找第一個位於工作範圍內的目標
