...
class PedicelSkeletonizer:
    ...
    """用兩次 BFS 找骨架圖的最長路徑（樹的直徑），回傳像素路徑。純 2D 骨架運算，
    不知道哪端是果實端——這邊只管找出最長路徑本身，方向判斷需要深度/世界座標
    （比對哪端真正離配對到的番茄近），那是 detector.py 的事，不是這裡的職責。
    reverse=True 時把路徑反過來排（由呼叫端決定，這裡只負責照做）。
    ...
    @classmethod
    def order_skeleton_path(cls, skeleton: np.ndarray, reverse: bool = False):
    ...
    """從果實端 (path[0]) 沿路徑走 (ratio_min+ratio_max)/2 的比例找抓取點——固定比例，
    不看實際物理距離，不管果梗多長都保證落在 C 端附近同一個相對位置。適合果梗普遍偏短
    （例如量出來都在 2cm 以內）的情境，用固定物理距離很容易逼近甚至超過整根果梗長度。"""
    @staticmethod
    def find_grasp_point_by_ratio(ordered_path: list, ratio_min: float, ratio_max: float):
        n = len(ordered_path)
        if n < 2:
            return None
        ratio = (ratio_min + ratio_max) / 2.0
